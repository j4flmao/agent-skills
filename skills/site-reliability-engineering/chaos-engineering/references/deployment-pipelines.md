# Deployment Pipelines for Chaos
## Purpose
How to integrate chaos engineering into continuous integration and continuous deployment pipelines safely using Canary Chaos and automated aborts.

## Pipeline Stages

### Stage: Build
In the `Build` phase, specific chaos actions are permitted.
```yaml
build_job:
  stage: build
  script:
    - echo "Running Build tests"
    - chaos run ./experiments/build-experiment.json
  allow_failure: false
```

### Stage: Unit Test
In the `Unit Test` phase, specific chaos actions are permitted.
```yaml
unit test_job:
  stage: unit test
  script:
    - echo "Running Unit Test tests"
    - chaos run ./experiments/unit test-experiment.json
  allow_failure: false
```

### Stage: Integration Test
In the `Integration Test` phase, specific chaos actions are permitted.
```yaml
integration test_job:
  stage: integration test
  script:
    - echo "Running Integration Test tests"
    - chaos run ./experiments/integration test-experiment.json
  allow_failure: false
```

### Stage: Chaos-Staging
In the `Chaos-Staging` phase, specific chaos actions are permitted.
```yaml
chaos-staging_job:
  stage: chaos-staging
  script:
    - echo "Running Chaos-Staging tests"
    - chaos run ./experiments/chaos-staging-experiment.json
  allow_failure: false
```

### Stage: Canary-Deploy
In the `Canary-Deploy` phase, specific chaos actions are permitted.
```yaml
canary-deploy_job:
  stage: canary-deploy
  script:
    - echo "Running Canary-Deploy tests"
    - chaos run ./experiments/canary-deploy-experiment.json
  allow_failure: false
```

### Stage: Chaos-Canary
In the `Chaos-Canary` phase, specific chaos actions are permitted.
```yaml
chaos-canary_job:
  stage: chaos-canary
  script:
    - echo "Running Chaos-Canary tests"
    - chaos run ./experiments/chaos-canary-experiment.json
  allow_failure: false
```

### Stage: Prod-Deploy
In the `Prod-Deploy` phase, specific chaos actions are permitted.
```yaml
prod-deploy_job:
  stage: prod-deploy
  script:
    - echo "Running Prod-Deploy tests"
    - chaos run ./experiments/prod-deploy-experiment.json
  allow_failure: false
```

### Stage: Continuous-Chaos
In the `Continuous-Chaos` phase, specific chaos actions are permitted.
```yaml
continuous-chaos_job:
  stage: continuous-chaos
  script:
    - echo "Running Continuous-Chaos tests"
    - chaos run ./experiments/continuous-chaos-experiment.json
  allow_failure: false
```

### Stage: Post-Deploy-Verify
In the `Post-Deploy-Verify` phase, specific chaos actions are permitted.
```yaml
post-deploy-verify_job:
  stage: post-deploy-verify
  script:
    - echo "Running Post-Deploy-Verify tests"
    - chaos run ./experiments/post-deploy-verify-experiment.json
  allow_failure: false
```

### Stage: Nightly-Chaos
In the `Nightly-Chaos` phase, specific chaos actions are permitted.
```yaml
nightly-chaos_job:
  stage: nightly-chaos
  script:
    - echo "Running Nightly-Chaos tests"
    - chaos run ./experiments/nightly-chaos-experiment.json
  allow_failure: false
```

## Automated Abort Matrix
| Metric | Threshold | Action |
|---|---|---|
| Error Rate Tier 1 | > 1% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 2 | > 2% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 3 | > 3% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 4 | > 4% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 5 | > 5% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 6 | > 6% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 7 | > 7% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 8 | > 8% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 9 | > 9% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 10 | > 10% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 11 | > 11% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 12 | > 12% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 13 | > 13% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 14 | > 14% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 15 | > 15% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 16 | > 16% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 17 | > 17% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 18 | > 18% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 19 | > 19% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 20 | > 20% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 21 | > 21% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 22 | > 22% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 23 | > 23% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 24 | > 24% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 25 | > 25% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 26 | > 26% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 27 | > 27% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 28 | > 28% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 29 | > 29% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 30 | > 30% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 31 | > 31% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 32 | > 32% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 33 | > 33% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 34 | > 34% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 35 | > 35% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 36 | > 36% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 37 | > 37% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 38 | > 38% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 39 | > 39% | Abort pipeline and trigger PagerDuty |
| Error Rate Tier 40 | > 40% | Abort pipeline and trigger PagerDuty |
