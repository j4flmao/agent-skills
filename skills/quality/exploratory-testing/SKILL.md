...
|---|---|---|---|
| Session length | 60-90 minutes (focused) | 2-3 hours (deep dive) | Feature complexity, tester experience |
| Charter format | Mission-based (what to explore) | Scenario-based (how to use) | Team preference, test maturity |
| Documentation | Session sheet (structured) | Mind map (visual) | Reporting requirements, personal style |
| Team setup | Paired testing (collaborative) | Solo + debrief (independent) | Team size, skill levels |

### Heuristic Selection by Application
- Web app → Page heuristics (forms, navigation, links, errors)
- Mobile app → Gesture heuristics (swipe, pinch, rotate, tap)
- API → Input heuristics (boundary, type, format, encoding)
- Data pipeline → Flow heuristics (ordering, timing, failure, recovery)