...
## Purpose
Securing APIs: JWT, CSRF, XSS, Rate Limiting, and CORS in Swift.
...
func authenticate(req: Request) async throws -> UserPayload {
    let token = req.headers.bearerAuthorization?.token ?? ""
    return try req.jwt.verify(token, as: UserPayload.self)
}
```
