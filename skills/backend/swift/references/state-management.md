...
## Purpose
Managing application state, distributed caching, and session storage.
...
func cacheUser(req: Request, user: User) async throws {
    let redis = req.redis
    let data = try JSONEncoder().encode(user)
    try await redis.set("user:\(user.id!)", to: String(data: data, encoding: .utf8)!)
}
```
