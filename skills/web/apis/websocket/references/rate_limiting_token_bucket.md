...
## Token Bucket Algorithm Formulation

The token bucket algorithm can be formally defined mathematically as follows:

Let `r` be the rate at which tokens are added to the bucket (tokens per second).
Let `b` be the maximum capacity of the bucket (maximum burst size).
Let `t` be the current time and `t_last` be the time of the last request.
Let `T` be the current number of tokens in the bucket.

1. Upon a new request at time `t`:
   Calculate the number of new tokens generated since the last request:
   `tokens_added = (t - t_last) * r`

2. Update the bucket capacity:
   `T = min(b, T + tokens_added)`

3. Check if the request can be accommodated (cost of request is `c`):
   if `T >= c`:
       `T = T - c`
       allow request
   else:
       drop/throttle request

This mathematically ensures that the average rate does not exceed `r` while allowing transient bursts up to `b`.
