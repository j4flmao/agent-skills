# Ultimate Deep Dive: Testing Strategies in data-lakehouse
...
When optimizing for testing-strategies in data-lakehouse, the interaction between the kernel and user space must be minimized. System calls such as `epoll_wait` or `io_uring` should be utilized for asynchronous I/O. Furthermore, memory alignment and CPU cache locality (L1/L2 cache hits) significantly out-weigh algorithmic improvements at scale.
