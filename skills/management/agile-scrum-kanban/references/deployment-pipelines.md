# Ultimate Deep Dive: Deployment Pipelines in agile-scrum-kanban
...
When optimizing for deployment-pipelines in agile-scrum-kanban, the interaction between the kernel and user space must be minimized. System calls such as `epoll_wait` or `io_uring` should be utilized for asynchronous I/O. Furthermore, memory alignment and CPU cache locality (L1/L2 cache hits) significantly out-weigh algorithmic improvements at scale.
